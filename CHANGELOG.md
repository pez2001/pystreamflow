...
- Editor: thumbnail of the newest image/video frame on the node tile, switchable per node from the context menu (saved as the editor-only config key `_preview`); nodes the growing tile would overlap move down
