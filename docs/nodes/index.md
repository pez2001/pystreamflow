...
## Media Nodes
**Port types.** Many ports declare what they carry – `text`, `number`, `json`, `image` (video frames count as images), `audio`, `video` or `media` (any of the three); the rest accept anything. The editor colours each port's dot by its type (legend in the empty details panel) and draws a wire between mismatched ports in red, e.g. an image into a text node. It still connects – the check only warns (also in the `/nodes/connect` and `/workflows` responses, the MCP `connect_nodes` result and the server log).
...
## Video Nodes
Need PyAV (`pip install 'pystreamflow[video]'`, the default backend) or, as a fallback, the `ffmpeg`/`ffprobe` executables; each node picks with `backend: auto|pyav|ffmpeg`. The ffmpeg fallback can't emit the audio track or PNG frames.

Frames travel as `video_frame` items holding a JPEG (default) or PNG, so every image node works on them unchanged. Their `meta` has `pts` (seconds), `index`, `fps`, `width`, `height`, `keyframe` (PyAV) and `last` on the final frame of a file.

- **VideoDecodeNode** – A `video` item on `in` (an upload, a MediaFileInputNode file) or `source` (a file path, or a stream URL such as `rtsp://…`/`http://…`, reopened after `reconnect_s` seconds if it fails or ends – default 5, 0 = give up) → frames on `out`. `fps` (0 = every frame; frames are picked on a fixed 1/fps grid, skipped frames are never encoded), `max_side` (scale down), `frame_format` (`jpeg`/`png`), `quality` (JPEG, default 85). `audio: true` also emits the soundtrack as `audio_chunk` WAV items on the `audio` port (`audio_chunk_ms`, default 500; `audio_sample_rate`, 0 = original). Output edges block when full, so a file is never decoded faster than it is consumed; for a live camera give the edge `buffer: {drop_policy: drop_oldest}` to always get the newest frames.
- **VideoFrameSampleNode** – Thins a frame stream: `mode` `every_n` (`n`, default 5), `fps` (at most `fps` frames per second of stream time, by `pts`) or `keyframes`.
- **VideoEncodeNode** – Frames (`video_frame` or `image` items on `in`), optionally with sound on the `audio` port, → a `video` item: `format` `mp4` (H.264 + AAC, default) or `webm` (VP9 + Opus), `crf` (quality, lower = better; 23/32), `fps` (default: from the frames). Timing follows the frames' `pts`, so a 1 fps sample plays back in real time. A file ends every `segment_s` seconds (0 = off), on a `last` frame (plus `audio_grace_s`, default 0.5, when audio is wired), on anything on the `flush` port, after `flush_idle_s` without input (default 2) and when the node stops. All frames of a file take the first frame's size.
- **VideoInfoNode** – Emits `{mime, size, filename, container, duration, bit_rate, video_codec, width, height, fps, frames, pix_fmt, audio_codec, sample_rate, channels}`.
- **VideoThumbnailNode** – One frame at `at_s` seconds (default 1.0) or `at_percent` of the duration as a JPEG `image` item; `max_side`, `quality`.
- Typical chain (from the plan): VideoDecode (`fps: 1`) → ImageResize (`max_side: 512`) → LMStudioNode (vision) → text nodes / MQTT.
