...
- **DirectoryInputNode** – Watches a directory and emits newly-seen entries on two separate ports: files on `files`, subdirectories on `dirs`. Config: `path`, `recursive` (bool, default `false` – walk nested subdirectories too), `poll_interval`. Both `path` and `recursive` can be updated live via an attribute wire.
